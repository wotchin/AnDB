...
def a_test_wal_manager():
    ...
    for i in range(iterations):
        ...
        assert record._header.xid == i
        assert record._header.action == WALAction.HEAP_INSERT
        assert record.data == (str(i) * 100).encode()

        if i % 5 == 0:
            commit_record = next(record_generator)
            assert commit_record._header.xid == i
            assert commit_record._header.action == WALAction.COMMIT
            assert commit_record.data == b''

    commit_record = next(record_generator)
    assert commit_record._header.xid == 100
    assert commit_record._header.action == WALAction.COMMIT
    assert commit_record.data == b''
